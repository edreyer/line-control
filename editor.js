 ...
 * Demo: http://www.jqueryscript.net/demo/Responsive-WYSIWYG-Text-Editor-with-jQuery-Bootstrap-LineControl-Editor/
 * GitHub: https://github.com/suyati/line-control
 * Fork: https://github.com/edreyer/line-control
*/
	...
	var methods = {
				...
				var menuItem;
	       		if(item in menuGroups){
	       			...
	       			for(var index=0;index<menuGroups[item].length;index++){
	       				...
	       				if(settings[value]){
       						menuItem = methods.createMenuItem.apply(this,[menuItems[value], settings[value], true]);
	       			...
	       			menuItem = methods.createMenuItem.apply(this,[menuItems[item], settings[item],true]);
		...
		createTableContext: function(event,cMenuUl){
			...
			cMenuUl.append($('<li/>',{class:"dropdown-submenu",css:{display:"block"}})
       						.append($('<a/>',{"tabindex":"-1", href:"#","text":"Row","onclick":"return false;"}))
   						...
   						.append($('<a/>',{"tabindex":"-1", href:"#","text":"Column","onclick":"return false;"}))
   						.append($('<ul/>',{class:"dropdown-menu"})
   								.append($('<li/>').append($('<a/>',{
										id:"tbl_addcolumn",
										"href":"#",
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			returnElement = (typeof returnElement !== 'undefined') ? returnElement : false;

			var menuWrapElement, menuElement, action;
			if(itemSettings.select){
				menuWrapElement = $("<div/>", {class:"btn-group"});
				menuElement 	= $("<ul/>", {class:"dropdown-menu"});
		    	...
		    	menuWrapElement = methods.createModal.apply(this,[itemSettings.modalId, itemSettings.modalHeader, itemSettings.modalBody, itemSettings.onSave]);
		    	menuElement = $("<i/>");
				...
				menuWrapElement = $("<a/>",{href:'#', onclick:"return false;", class:'btn btn-default'});
				menuElement = $("<i/>");
				if(itemSettings.icon)
					menuElement.addClass(itemSettings.icon);
				else
